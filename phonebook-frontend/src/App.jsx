...
const Notification = ({ message, errorMessage }) => {
  ...
  return (
    <div className={errorMessage ? "error-message" : "message"}>{message}</div>
  );
};
...
const Persons = ({ persons, handleDelete }) => {
  return (
    <>
      {persons.map((person) => (
        <p className="note" key={person.name}>
...
const App = () => {
  ...
  const handleSubmit = (event) => {
    ...
    const find = persons.find((p) => p.name.trim() == newName.trim());
    ...
    if (find !== undefined) {
      if (
        window.confirm(
          `${find.name} is already added to Phonebook, replace the old number with the new one?`
        )
      ) {
        phoneService
          .update(find.id, newPerson)
          .then((response) => {
